...
if __name__ == "__main__":
    while True:
        ...
        if mode == "file":
            ...
            if not filename:  # If no filename is provided, use the default
                filename = 'encoded_file.txt'
            
            try:
                with open(filename, 'r') as file:
                    encoded_text = file.read()
                break  # Exit the loop if reading the file is successful
            except FileNotFoundError:
                print(f"Error: The file '{filename}' was not found. Please try again.")
                
        elif mode == "text":
            encoded_text = input("Enter the text: ")
            break  # Exit the loop after successful text input
